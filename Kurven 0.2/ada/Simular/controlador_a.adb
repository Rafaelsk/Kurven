...
procedure controlador_a is
	...
	task body TareaControlador is		--se tiene que agregar kp y ki!!!
		Ts 			: constant Float 	:= Float'Value(Argument(1));			--RECORDAR QUE TIENE QUE SER 0.01 SEGUNDOS DE PERIODO DE MUESTREO PARA EL CONTROLADOR!!!!!
		...
		Hueco_Controlador_A 	: Socket_Type;
		Canal_Control_A		: Stream_Access;					-- Canal
	...
	begin
		...
		Direccion_Planta	 := (	Family_Inet,
	   						...
	   						Port_Type(50001 ) );
		Create_Socket (Hueco_Controlador_A);
		Connect_Socket (Hueco_Controlador_A, Direccion_Planta);
		Canal_Control_A := Stream (Hueco_Controlador_A);   -- Asocia un stream con el socket
   		t_inicio := Clock;
   		Siguiente := t_inicio;
   	loop
		E  := r - y;
		...
		Put_Line(u'img);
		String'Output( Canal_Control_A, "ACT:" & u'img );			-- Envía Acc. de control a la planta y solicita valor de la salida
		...
		String'Output( Canal_Control_A, "FIN:" );			-- Envía el mensaje de finalización a la Planta
		delay(3.0);
		Close_Socket(Hueco_Controlador_A);
		--Put_Line("Comunicacion del Controlador A finalizada.");
   end TareaControlador;
	...
	begin
		Create_Socket(Sock);
		Addr := (Family_Inet, Any_Inet_Addr, Port_Type( 50003)  );
		Bind_Socket(Sock, Addr);
		Listen_Socket(Sock);
		--Put_Line("Inicio Tarea Servidor del Controlador A");
