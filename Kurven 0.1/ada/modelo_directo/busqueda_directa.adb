...
package body busqueda_directa is
	...
	Procedure lee_archivo_de_datos(DIRECCION : in String) is
	   Arch   : File_Type;
	   Nombre : String := DIRECCION;
   ...
   procedure calcula_k_tau is
   	...
   	p1, p2, p3 : T_Punto;		-- Define 3 variables tipo punto, que contien 2 parámetros long_float
   begin
   	kd := long_float(v(n));
		...
		if (long_float(v(iv63)) = v63) then
			...
			p1.x := long_float(t(iv63-1));
			p1.y := long_float(v(iv63-1));
			p2.x := long_float(t(iv63));
			p2.y := long_float(v(iv63));
			p3 := parametros_recta( p1, p2 );
			taud := (v63-p3.y)/p3.x;
		end if;
   end calcula_k_tau;
	...
	function guarda_datos_modelo return Boolean is
	   Arch   : File_Type;
	   Nombre : String := "resultado_modelo_directo.txt";
